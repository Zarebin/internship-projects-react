...
function getCurrentEntry(state, action) {
  const currentEntry = {
...
function addEntry(state, action) {
  const currentEntry = getCurrentEntry(state, action);
  ...
  const monitorEnd = state.monitorExpression.slice(
    state.monitorCursor,
    state.monitorExpression.length
  );
...
function removeEntry(state) {
  const { expressionHistory } = state;
  ...
  const end = state.expression.slice(
    state.expression.lastIndexOf(lastEntry.value) + lastEntry.value.length,
    state.expression.length
  );
  state.expression = start + end;
  state.cursor -= lastEntry.value.length;

  const monitorStart = state.monitorExpression.slice(
    0,
    state.monitorExpression.lastIndexOf(lastEntry.monitorValue)
  );
  const monitorEnd = state.monitorExpression.slice(
    state.monitorExpression.lastIndexOf(lastEntry.monitorValue) + lastEntry.monitorValue.length,
    state.monitorExpression.length
  );
  ...
  switch (currentEntry.id) {
    case 'close-parenthesis':
      if (lastEntry !== undefined) {
        if (
          lastEntry.id === 'open-parenthesis' ||
          lastEntry.id === 'close-parenthesis' ||
          lastEntry.type === 'operator'
        ) {
          return false;
        }
        if (lastEntry.id === 'point') {
          return false;
        }
      }
      break;
...
function performSmartAddition(state, action) {
  ...
  if (lastEntry !== undefined) {
    // Reset most of the state when gets equals as last entry
    // except when gets an operator or percentage after that
    if (
      lastEntry.id === 'equals' &&
      !(currentEntry.type === 'operator' || currentEntry.id === 'percentage')
    ) {
      state.expression = '';
    ...
    // Checks if last entry is an operator and gets another operator after
    // it then replaces last entry with the new operator
        ...
        try {
          ...
          const result = Number(
            math
              ...
              .toFixed(CONSTANTS.NUMBER_PRECISION)
          );
          if (!Number.isNaN(result)) {
            ...
            state.primaryMonitor = state.monitorExpression;
            state.expressionHistory.push(getCurrentEntry(state, action));
          }
        } catch (error) {
    ...
    clearEntry(state) {
      ...
      if (lastEntry !== undefined) {
        if (lastEntry.id === 'equals') {
          ...
          lastEntry.type === 'operand' ||
          lastEntry.type === 'operator' ||
          lastEntry.type === 'function'
        ) {
          removeEntry(state);
        }
      }
...
export const { addToExpression, solveExpression, addCloseParenthesis, clearEntry } =
